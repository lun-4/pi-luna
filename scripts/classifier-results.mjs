...
if (rows.length < 2) {
  ...
  const columns = ["model", "prompt", "tests", "approved", "denied", "correct", "accuracy", "tokens", "cost"];
  const widths = Object.fromEntries(
    columns.map((column) => [column, Math.max(column.length, ...table.map((row) => row[column].length))]),
  );
